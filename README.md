...
Whenever a user stores credentials in Edge (via the built-in password manager), all credentials are stored in plaintext in the parent Edge process memory. On shared systems (e.g. terminal servers) an attacker with admin rights can read them across all logged-on and disconnected users.
